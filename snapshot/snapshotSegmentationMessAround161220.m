...
%[fnames, lims, flabel] = postIlastik({datapath{1}}, {dataset{1}(1)});

saveIntermediates = false;%true;

%%
for fi = 122%:numel(filepath) 
...
%     %-----END HACK------
    ...
    for i = 1:CC.NumObjects
        ...
        cellPixels = setdiff(CC.PixelIdxList{i}, bdryPixelIdxList);
        %cellPixels = CC.PixelIdxList{i};
        FatRaw =  mean(Fat(cellPixels));
        DsRaw = mean(Ds(cellPixels));
...
%                                     'bdryFI', 'bdryDI', 'assignedBdry')
    ...
    save(fullfile(filepath{fi},segResultsDir,[flabel{fi} '_seg_exclBdry']), 'cellLayer');
