...
s = strsplit(s{end},'.');
zslice = str2double(s{1});
stack = readStack(fullfile(filepath,'..'),fullstackfname);
basalz = min(zslice+4,size(stack,3));
...
for i = 1:nSnakes
    ...
    plot(snake(:,1) + s*FatSgn*nx{i}', snake(:,2) + s*FatSgn*ny{i}', '-','LineWidth', lw, 'Color', 'g')
...
for fi = 1:numel(files)
    ...
    for i = 1:S.nSnakes
        ...
        if S.resolution < cutoffres...% && ~(any(S.dsCyt{i}(1:2) <= 1.5) && abs(DsGrad) < 0.25)...
            && ~(S.dsCyt{i}(5) < 0.2)%|| S.fatCyt{i}(5) < 0.9)
...
if saveResults 
    fname = fullfile(datapath, ['gradientVsPolarityNew_res' num2str(cutoffres) '.png']);
    saveas(gcf, fname);
    fname = fullfile(datapath, ['gradientVsPolarityNew_res' num2str(cutoffres) '.fig']);
    saveas(gcf, fname);
end
...
axis([binEdge(1) binEdge(end) 0 max(nAligned)+max(nNotAligned)+2])
xlabel('dFat')
